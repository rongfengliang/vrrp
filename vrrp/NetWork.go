...
import (
	...
	"github.com/rongfengliang/vrrp/logger"
...
func (nd *IPv6AddrAnnouncer) AnnounceAll(vr *VirtualRouter) error {
	for key := range vr.protectedIPaddrs {
		var multicastgroup, errOfParseMulticastGroup = ndp.SolicitedNodeMulticast(netip.MustParseAddr(string(key[:])))
		if errOfParseMulticastGroup != nil {
			...
			var msg = &ndp.NeighborAdvertisement{
				Override:      true,
				TargetAddress: netip.MustParseAddr(net.IP(key[:]).String()),
				Options: []ndp.Option{
...
func (ar *IPv4AddrAnnouncer) AnnounceAll(vr *VirtualRouter) error {
	...
	for k := range vr.protectedIPaddrs {
		...
		packet.SenderIP = netip.MustParseAddr(net.IP(k[:]).String())
		packet.TargetHardwareAddr = BaordcastHADDR
		packet.TargetIP = netip.MustParseAddr(net.IP(k[:]).String())
