...
func (r *VirtualRouter) fetchVRRPPacket() {
	for {
		logger.GLoger.Printf(logger.INFO, "Read Message")
		if packet, errofFetch := r.iplayerInterface.ReadMessage(); errofFetch != nil {
			...
			if r.vrID == packet.GetVirtualRouterID() {
				logger.GLoger.Printf(logger.INFO, "VirtualRouter.packetQueue End")
		...
		logger.GLoger.Printf(logger.INFO, "VirtualRouter.fetchVRRPPacket End")
